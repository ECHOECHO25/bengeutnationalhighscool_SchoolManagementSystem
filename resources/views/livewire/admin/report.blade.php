        ...
        <div class="bg-white p-5 rounded-2xl">
            <div>
                <x-filament::input.wrapper class="w-64">
                    <x-filament::input.select wire:model.live="selectedReport">
                        <option value="draft">Select Report</option>
                        <option value="reviewing">Students</option>
            ...
            <div class="mt-5">
                @if ($selectedReport)
                    <x-filament::input.wrapper class="w-64">
                        <x-filament::input.select wire:model.live="schoolYear">
                            <option value="draft">Select School Year</option>
                            @foreach ($school_years as $item)
                                @php

                                @endphp
                                <option value="{{ $item->id }}">S.Y
                                    {{ \Carbon\Carbon::parse($item->start_date)->format('Y') . ' - ' . \Carbon\Carbon::parse($item->end_date)->format('Y') }}
                                </option>
                            @endforeach
                        </x-filament::input.select>
                    </x-filament::input.wrapper>
        ...
        <div class="mt-5">
            @if ($schoolYear)
                <div class="mt-10 bg-white rounded-xl p-10">
                    <x-filament::button icon="heroicon-m-printer" icon-position="after" @click="printOut($refs.printContainer.outerHTML);">
                        Print Report
                    </x-filament::button>
                   <div x-ref="printContainer">
                     <h1 class="font-semibold mt-5 text-gray-700">STUDENT SY: {{ $year_name ?? '' }}</h1>
                    <div class="flex flex-col">
                       <table id="example" class="table-auto mt-3" style="width:100%">

                                <thead class="font-normal">
                                    <tr>
                                        <th class="border w-20  text-left px-2  font-bold text-gray-700"></th>
                                        <th class="border  text-left px-2  font-bold text-gray-700">FIRSTNAME
                                        </th>
                                        <th class="border  text-left px-2  font-bold text-gray-700">LASTNAME
                                        </th>
                                        <th class="border  text-left px-2  font-bold text-gray-700">GRADE LEVEL
                                        </th>
                                        <th class="border  text-left px-2  font-bold text-gray-700">SECTION
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="">
                                    ...
                                    @endphp
                                     @foreach ($students as $item)
                                         <tr>
                                            <td class="border  text-gray-700  px-2 ">
                                              {{$i++}}
                                            </td>
                                            <td class="border  text-gray-700  px-2 ">
                                              {{$item->firstname}}
                                            ...
                                            <td class="border  text-gray-700  px-2 ">
                                              {{$item->studentClassrooms->first()->classroom->building_number. ' - ' . $item->studentClassrooms->first()->classroom->section ?? ''}}
                                            </td>
                ...
                <div class="grid place-content-center mt-10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-96" viewBox="0 0 797.5 834.5"
                        ...
                        <path
                            ...
                            transform="translate(-201.25 -32.75)" fill="#2f2e41" />
                        <circle cx="559" cy="744.5" r="43" fill="#f59e0b" />
                        <circle cx="54" cy="729.5" r="43" fill="#f59e0b" />
                        <circle cx="54" cy="672.5" r="31" fill="#f59e0b" />
                        <circle cx="54" cy="624.5" r="22" fill="#f59e0b" />
                    </svg>
                    <h1 class="text-center text-white mt-5">Select Report to generate</h1>
                </div>
            @endif
        </div>
    </div>
